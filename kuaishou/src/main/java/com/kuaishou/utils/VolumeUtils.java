...
public class VolumeUtils {
    public static List<String> downloaddata = null;
...
//        Imgproc.cvtColor(rgbMat,rgbMat,Imgproc.COLOR_RGB2GRAY);
        Imgproc.threshold(dst1.get(2), rgbMat, 20, 255, Imgproc.THRESH_TOZERO);
...
//        Imgproc.medianBlur(picture,picture,5);
    ...
    public double[] imageDeal(Mat photo2, double[] V) {
        int q = 0;
        int th =10;
        int th1 = 25;
...
//                V[4] = 1;
    ...
    private static boolean isInlines(Point a, Point b, Point c, Point d) {
        ...
        if (k0 != 0) {
            y = k0 * c.x + b0;
            y1 = k0 * d.x + b0;
            if (abs(y - c.y) < 2 || abs(y1 - d.y) < 2) {
                flag = true;
            }
        } else {
            if (abs(a.x - c.x) < 2 || abs(a.x - d.x) < 2) {
                flag = true;
            }
        }
        return flag;
    ...
    private static void findLines(Bitmap L, double p, double a, double b, double[][] xx, double[][] yy) {
        int w = L.getWidth();
        int h = L.getHeight();
        int m;
        int n;
    ...
    private static void node(double[] X1, double[] X2, double[] Y1, double[] Y2, Point p) {
        double K2;
        double b2;
        double K1;
        double b1;
    ...
    private static int max(double x1, double x2, double x3, double x4) {
        int max;
        max = (int) ((x1 > x2) ? x1 : x2);
    ...
    private static int min(double x1, double x2, double x3, double x4) {
        int min;
        min = (int) ((x1 < x2) ? x1 : x2);
